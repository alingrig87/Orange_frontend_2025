...
function afiseazaCarduri(produse) {
...
afiseazaCarduri(produse);

const produseIntre5si7lei = produse.filter(
	(produs) => produs.pret >= 5 && produs.pret <= 7
);

const checkboxFiltru = document.getElementById('filtru');

checkboxFiltru.addEventListener('change', gestioneazaFiltrare);

function gestioneazaFiltrare(e) {
	if (e.target.checked === true) {
		afiseazaCarduri(produseIntre5si7lei);
	} else {
		afiseazaCarduri(produse);
	}
}
